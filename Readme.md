...
1. - [x] Implement load testing using `artillery`
   - [x] 1. Create a PR against `master` of your fork including artillery
   - [x] 2. Ensure that load testing is able to be run using `npm test:load`. You can consider using a tool like `forever` to spin up a daemon and kill it after the load test has completed.
   - [x] 3. Test all endpoints under at least `100 rps` for `30s` and ensure that `p99` is under `50ms`
